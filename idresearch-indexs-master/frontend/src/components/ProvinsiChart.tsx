...
import {getProvinsiStats} from '../repositories/DaerahGeojson';

am4core.useTheme(am4themes_animated);

const ProvinsiChart = () => {
  const [loading, setLoading] = useState(false);
  const renderChart = (dataTopic: Array<Topic>) => {
    dataTopic = sortBy(dataTopic, ['TotalInformation'], ['desc']).reverse();

    let chart = am4core.create('chartdivprovinsi', am4charts.XYChart);

    chart.data = dataTopic;
    var topicAxis = chart.xAxes.push(new am4charts.CategoryAxis());
    topicAxis.dataFields.category = 'Label';
    topicAxis.renderer.grid.template.location = 0;
    topicAxis.renderer.minGridDistance = 30;
    topicAxis.renderer.labels.template.rotation = -90;
    topicAxis.renderer.labels.template.fontSize = 10;
    ...
    series.dataFields.valueY = 'TotalInformation';
    series.dataFields.categoryX = 'Label';

    series.name = 'Provinsi';
    series.tooltipText = '{Label}: [b]{valueY}[/]';
  ...
  return (
    <>
      <h3 className="font-bold text-2xl ml-4">
        Total data per provinsi (50 Teratas)
      </h3>
      ...
      <div id="chartdivprovinsi" style={{width: '100%', height: '300px'}}></div>
    </>
  );
};
